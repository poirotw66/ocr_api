...
from UVDoc.model import UVDocnet
import cv2
...
def determine_scan_img(img):
    channels = cv2.split(img)
    if cv2.countNonZero(channels[0]-channels[1])==0 and cv2.countNonZero(channels[0]-channels[2])==0:
        return True
    else:
        return False
